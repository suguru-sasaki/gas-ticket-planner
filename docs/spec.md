...
- **Backlog上のチケットをガントチャートで可視化**（本来の運用形態）
- テンプレートベースで親子チケット構造を定義
- Backlog連携を前提とした抽象化設計

> **注意**: 本システムのチケット管理はBacklog上で行います。スプレッドシート上の「チケット管理」シートは、Backlog連携実装前のスタンドアロン動作用（テスト・デモ用）です。
...
| チケット管理 | チケットデータの保存（スタンドアロン版用） | システム管理（Backlog連携時は不使用） |
...
本システムの本来の運用形態はBacklog連携です。スプレッドシートはガントチャート可視化専用として使用し、チケット管理はBacklog上で行います。

### 9.2 Backlog連携機能（実装予定）

- **チケット作成**: Backlogへ課題を登録（スプレッドシートには保存しない）
- **データ取得**: Backlogからチケット情報をインポート
- **状態同期**: Backlogの状態変更を反映
- **双方向同期**: スプレッドシート上の変更をBacklogに反映（オプション）
