...
export class SheetInitializer {
  ...
  private initializeUsageSheet(): void {
    ...
    const content = [
      ...
      ['   - 「説明文を含める」にチェックを入れると、説明文列が追加されます'],
      [''],
      ['■ ガントチャートの色分け'],
      ['・未対応: 赤系（#ee7f77）'],
      ['・処理中: 青系（#4389c5）'],
  ...
  private initializeSettingsSheet(): void {
    ...
    const settingsData = [
      ...
      [SETTING_KEYS.childColorProcessed, DEFAULT_SETTINGS.childColorProcessed],
      [SETTING_KEYS.childColorCompleted, DEFAULT_SETTINGS.childColorCompleted],
      [SETTING_KEYS.overdueColor, DEFAULT_SETTINGS.overdueColor],
